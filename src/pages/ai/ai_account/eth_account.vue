...
export default {
  ...
  methods: {
    ...
    // 判断默认账户是否有当前币种（ETH）（type === 2），如果没有则自动切换为下一个有当前币种的账户
    initAccount () {
      // 判断默认账户是否有当前币种
    ...
    getInfo () {
      this.$axios.get('v1/ai/account')
        .then(({ data }) => {
          if (data.status === 200) {
            this.all_num = (data.data.ai_eth + data.data.profit_eth).toFixed(4);
            this.principal = data.data.ai_eth;
            this.earnings = data.data.profit_eth;
            this.lastWeek = data.data.ai_eth_last_week_profit;
          }
        });
    },
    // 获取记录
    ...
    rollOut () {
      if (this.defaultIsCoin) {
        ...
        this.$global.alertOptions(`当前默认账户没有ETH币种，请先导入或切换账户后进行操作`);
      }
    }
  },
  components: {
